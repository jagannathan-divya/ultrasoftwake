...
#          Bond number Bo, and scaled shear wave speed cs.
# Author: Divya Jagannathan (Harvard University 2025)
# ===============================================================


def mycolor():
    basic = plt.get_cmap('seismic')
    bright_red = (1.0, 0.0, 0.0, 1.0)
    colors = [bright_red, bright_red, basic(0.5), basic(0.25), basic(0.25)]
...
def integrand(kx, ky, Fr, Bo, cs):
    ...
    # Regularization of Doppler Shift
    dpplr = (Fr*kx + 1j*eps)
    f2_num = k*(dpplr**2)
...
# (NOTE: k used in the integral should be radians per unit length; hence multiplied by 2*pi)
kx = 2*np.pi*np.fft.fftfreq(Nk, d=Lxy/Nk)
ky = kx
...
# (!!) CAUTION: Taking real part after performing the inverse Fourier transform
